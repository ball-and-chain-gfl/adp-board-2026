...
.ramp{width:190px;height:12px;border-radius:6px;background:linear-gradient(90deg,#0f9d4f,#54c47f,#2b323d,#e8736a,#d0342c)}
...
<div class="bar">
  ...
  <div class="legend"><span class="chip p">3 of 3</span><span class="chip b">2 of 3</span><span>sites earlier than ESPN</span>
...
the ESPN cell on that row (full colour at 3 rounds), so you can read the practical question directly: green means you can wait that many rounds and still get him, red means you'd have to
come up that many rounds to beat the field. A big pick gap that doesn't cross a round boundary is usually not worth changing your plan over.<br>
<b>The ESPN cell itself lights up</b> when ESPN's number is higher than its counterparts &mdash; meaning ESPN is the cheapest room in the market for that player.
<span class="chip p">Purple</span> = all three sites take him earlier (unanimous). <span class="chip b">Blue</span> = two of the three do.
Both shade with how far off ESPN is, on the same &plusmn;25 scale as the green and red cells, so a deep purple is a big consensus discount and a faint blue is a coin flip.
Requires the sites to be reporting, so kickers and defenses never qualify.<br>
<b>Reading the colors.</b> The small number is the gap from ESPN.
<span style="color:#54c47f"><b>Green = the player goes EARLIER on that site than on ESPN</b></span> &mdash; the rest of the market values him higher than ESPN does, so he's a relative bargain at his ESPN price.
<span style="color:#e8736a"><b>Red = the player goes LATER on that site than on ESPN</b></span> &mdash; ESPN is paying up; you can likely wait.
...
function recompute(){
  DATA.forEach(r=>{
    ...
    // how many counterparts take him EARLIER than ESPN, and by how much on average
    const neg = KEYS.map(k=>r["g_"+k]).filter(g=>g!==null && g<0);
    r.nUnder = neg.length;
    r.underAvg = neg.length ? +(neg.reduce((a,b)=>a+b,0)/neg.length).toFixed(1) : null;
...
// gap = other site - ESPN.  negative => goes earlier elsewhere => GREEN
function color(d){
  if(d===null) return "";
...
// ESPN cell highlight: 3 of 3 sites earlier => purple, exactly 2 => blue, scaled by how far off ESPN is
function hlColor(r){
  if(r.nUnder < 2) return null;
  const rgb = r.nUnder >= 3 ? "138,74,226" : "31,111,235";
  const a = (0.28 + 0.72*Math.min(1, Math.abs(r.underAvg)/CAP)).toFixed(3);
